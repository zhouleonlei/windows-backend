...
void LinearLayout::LayoutHorizontal( LayoutLength left, LayoutLength top, LayoutLength right, LayoutLength bottom )
{
  ...
  auto childSpace = height - static_cast<int>( padding.top + padding.bottom );
  ...
  for( unsigned int i = 0; i < count; i++)
  {
    ...
    if( childLayout != nullptr )
    {
      ...
      childTop = LayoutLength( padding.top ) + ( ( childSpace - childHeight ) / 2 ) + static_cast<int>( childMargin.top - childMargin.bottom );

      childLeft += childMargin.start;
      childLayout->Layout( childLeft, childTop, childLeft + childWidth, childTop + childHeight );
      childLeft += childWidth + static_cast<int>( childMargin.end ) + mCellPadding.width;
    }
  }
}
...
void LinearLayout::LayoutVertical( LayoutLength left, LayoutLength top, LayoutLength right, LayoutLength bottom )
{
  ...
  // Space available for child
  auto childSpace = width - static_cast<int>( padding.start + padding.end );
  auto count = GetChildCount();
  ...
  for( unsigned int childIndex = 0; childIndex < count; childIndex++)
  {
    ...
    if( childLayout != nullptr )
    {
      ...
      childLeft = LayoutLength( padding.start ) + ( childSpace - childWidth ) / 2 + static_cast<int>( childMargin.start - childMargin.end );

      childLayout->Layout( childLeft, childTop, childLeft + childWidth, childTop + childHeight );
      childTop += childHeight + static_cast<int>( childMargin.bottom ) + mCellPadding.height;
    }
  }
}

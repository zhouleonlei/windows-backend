...
YGSize FlexLayout::OnChildMeasure( YGNodeRef node,
  ...
  auto measuredWidth = childLayout->GetMeasuredWidth() - static_cast<int>( padding.end + padding.start );
  auto measuredHeight = childLayout->GetMeasuredHeight() - static_cast<int>( padding.bottom + padding.top );
...
#else
  YGSize ret;
  ret.width = measuredWidth;
  ret.height = measuredHeight;
  return ret;
#endif
}

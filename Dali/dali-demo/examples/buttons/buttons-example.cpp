...
class ButtonsController: public ConnectionTracker
{
 public:
  ...
  void Create( Application& application )
  {
    ...
    char tempath[1000];
    GetCurrentDirectoryA( 1000, tempath );
...
{
  FILE *pf = NULL;
  std::string path = "dali-env/1.txt";
  while( NULL == pf )
  {
    pf = fopen( path.c_str(), "r" );

    if( 100 < path.length() || NULL != pf )
    {
      break;
    }
  ...
  pf = fopen( "../dali-env/opt/share/com.samsung.dali-demo/res/images/background-gradient.jpg", "r" );
